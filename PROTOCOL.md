...
* An HEVC stream uses the identical framing (section 5.1) — at `pv` 4 it
  travels in a frame of type `0` like any other video (section 4.1), the
  codec being a property of the payload rather than of the frame;
  keyframes are prefixed with VPS, SPS and PPS, and a codec switch
  mid-connection is an ordinary stream change (section 5.2).
